...
- `zjctl status` and `focused` selection no longer get stuck at `Focused: none` after plugin reloads; focus is derived from Zellij client metadata (with a deterministic fallback when no client is marked “current”).
